...
*
* Description: This file is adapted from GPU implementation of Rodinia benchmark.
*
*/
...
void runTest( int argc, char** argv) 
{
	double time;
	double start_time;   
	int max_rows, max_cols, penalty, pair_num;
	...
	for (int n = 0; n<pair_num; ++n){
		for( int i=1; i< max_rows ; i++){    //please define your own sequence. 
       		input_itemsets[n][i*max_cols] = rand() % 20 + 1;
		}
	    for( int j=1; j< max_cols ; j++){    //please define your own sequence.
    	   input_itemsets[n][j] = rand() % 20 + 1;
		}
	...
	//printf("The size of each matrix: %d\n", sizeof(int)*size);
	//start_time = gettime();
	...
	//fprintf(stdout,"Total memory usage: %dMB\n", sizeof(int)*size*NUM_SEQ*2/1024/1024);
	start_time = gettime();
	...
	for (int n=0; n<pair_num; ++n){
		...
		for( int i = 1 ; i <= block_width ; i++){
			dimGrid.x = i;
			dimGrid.y = 1;
			needle_cuda_shared_1<<<dimGrid, dimBlock>>>(referrence_cuda[n], matrix_cuda[n] 		                                   			,max_cols, penalty, i, block_width); 
		}
		...
		for( int i = block_width - 1  ; i >= 1 ; i--){
			dimGrid.x = i;
			dimGrid.y = 1;
			needle_cuda_shared_2<<<dimGrid, dimBlock>>>(referrence_cuda[n], matrix_cuda[n]
