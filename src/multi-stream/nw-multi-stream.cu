...
void usage(int argc, char **argv)
{
    ...
    fprintf(stderr, "\t[--num_pair|-n <pair num>] - number of pairs per stream\n");
    fprintf(stderr, "\t[--device|-d <device num> ]- device ID\n");
...
int main(int argc, char **argv)
{
	...
	e_time = gettime();
	printf("Data transfer from GPU : %fs\n", e_time - s_time);
