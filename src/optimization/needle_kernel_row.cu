...
#define PAIR_IN_BLOCK 4	// number of pair in each block

__global__ void needleman_cuda_row(char *sequence_set1, char *sequence_set2, 
...
{ 
	...
	__shared__ int s_line1[PAIR_IN_BLOCK][STRIDE_SIZE+1];
	__shared__ int s_line2[PAIR_IN_BLOCK][STRIDE_SIZE+1];
	int *pl1, *pl2, *ptmp, *matrix;
		...
		for (j=start_pair_no; j<end_pair_no; ++j) {	// iteration between different loops
			...
			for ( k=0; k<load_stride; ++k ) {
				...
				if ( index_y<=seq1_len ) {			
					...
					matrix[index_y] = s_line1[j-start_pair_no][tid+1+k*blockDim.x] = penalty * index_y;
			...
			for (j=start_pair_no; j<end_pair_no; ++j) {
				...
				for ( k=0; k<load_stride; ++k ) {
					...
					if ( index_y<=seq1_len ) {		
						...
						if ( index_x&1 == 1)
							matrix[index_x*(seq1_len+1)+index_y] = s_line2[j-start_pair_no][tid+1+k*blockDim.x];
						else
							matrix[index_x*(seq1_len+1)+index_y] = s_line1[j-start_pair_no][tid+1+k*blockDim.x];
					}
				}
			}
			__syncthreads();
